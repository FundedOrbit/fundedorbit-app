...
export function nextMonthlyAnniversary(purchaseDateStr, todayStr) {
  ...
  // El primer cobro recurrente siempre es al cumplirse 1 mes de la compra,
  // nunca el mismo día en que compras (antes esto se calculaba mal y
  // marcaba el cobro como "hoy" si comprabas y consultabas el mismo día).
  let candidate = new Date(purchase.getFullYear(), purchase.getMonth() + 1, day);
...
export async function createAccountsBulk(userId, rows) {
  const { data, error } = await supabase
    .from("accounts")
...
export function computeTopStats(list) {
  ...
  // La tasa de aprobación debe ser histórica: una cuenta que llegó a pasar (passed_date)
  // sigue contando como aprobada aunque después se queme o se reestablezca.
  let numAprobadasHist = 0,
    numQuemadasSinAprobar = 0;
  ...
  list.forEach((acc) => {
    ...
    if (acc.passed_date) {
      numAprobadasHist++;
    } else if (acc.status === "quemada") {
      numQuemadasSinAprobar++;
    }
  });

  const netProfit = totalRetirado - totalInvertido;
  const roiGlobal = totalInvertido > 0 ? (netProfit / totalInvertido) * 100 : 0;
  const resueltas = numAprobadasHist + numQuemadasSinAprobar;
  const tasaAprobacion = resueltas > 0 ? (numAprobadasHist / resueltas) * 100 : 0;
