...
export function computeTopWorst(list) {
  const withMetrics = list
    .filter((a) => (Number(a.purchase_cost) || 0) > 0 || (a.withdrawals || []).length > 0)
    .map((a) => ({ acc: a, m: accountMetrics(a) }));
  const sorted = [...withMetrics].sort((a, b) => b.m.roi - a.m.roi);
  return { top: sorted.slice(0, 5), worst: sorted.slice(-5).reverse() };
}
...
export function computeBreakdown(list) {
  ...
  return {
    byCompany: Object.values(companyMap).sort((a, b) => b.invertido - a.invertido),
    byMethod: Object.values(methodMap).sort((a, b) => b.invertido - a.invertido),
  };
}
